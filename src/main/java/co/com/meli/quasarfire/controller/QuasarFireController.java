...
public class QuasarFireController {
    ...
    public ResponseEntity<Response> topSecret(@RequestBody SatellitesReq requestSatellites) throws Exception {
        ...
        try {
            if(Objects.isNull(requestSatellites))
                throw new Exception("No se reciven datos para obtener el mensaje y la distancia");
            if(requestSatellites.getSatellites().size() != 3)
                throw new Exception("Se debén recibir solo los 3 satélites especificados");
    ...
    @ApiOperation(value = "Método para obtener todos o algun satelite en especifico", response = ResponseEntity.class)
    @ApiResponses(value = {
            @ApiResponse(code = 200, response = SatelliteEntity.class, message = "OK"),
            @ApiResponse(code = 404, response = Response.class, message = "Not found") })
    @GetMapping("satellites/{satellite_name}")
    public ResponseEntity<?> getSatellites(@PathVariable("satellite_name") String satelliteName) throws Exception{
        List<SatelliteEntity> satellites = new ArrayList<>();
        Response response = new Response();
        try{
            satellites= iQuasarFireService.getSatellite(satelliteName);
            return ResponseEntity.status(HttpStatus.OK).body(satellites);
        }catch (Exception e){
            response.setMessage(e.getMessage());
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(response);
        }
    ...
    @ApiOperation(value = "Método para eliminar la distancai y mensaje de los salélites o un solo satélite", response = ResponseEntity.class)
    @ApiResponses(value = {
            @ApiResponse(code = 200, response = SatelliteEntity.class, message = "OK"),
            @ApiResponse(code = 404, response = Response.class, message = "Not found") })
    @PostMapping("satellites/{satellite_name}")
    public ResponseEntity<?> deleteSatellites(@PathVariable("satellite_name") String satelliteName) throws Exception{
        List<SatelliteEntity> satellites = new ArrayList<>();
        Response response = new Response();
        try{
            satellites= iQuasarFireService.updateSatellites(satelliteName);
            return ResponseEntity.status(HttpStatus.OK).body(satellites);
        }catch (Exception e){
            response.setMessage(e.getMessage());
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(response);
